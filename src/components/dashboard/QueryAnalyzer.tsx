  ...
  const handleAnalyze = () => {
    ...
    createQueryMutation.mutate(
      { message, platform },
      {
        ...
        onError: () => {
          setFeedback({ type: "error", msg: "Failed to analyze query. Is the server running?" });
        },
      }
    );
  };
    ...
    <Card className="h-full border-primary/20 shadow-sm bg-gradient-to-br from-card to-primary/5">
      ...
      <CardContent className="p-4 space-y-4">
        <div className="space-y-3">
          ...
          <Button onClick={handleAnalyze} disabled={!message.trim() || createQueryMutation.isPending} className="w-full h-9 font-semibold">
            {createQueryMutation.isPending ? "Analyzing..." : "Analyze Query"}
            <Zap className="w-4 h-4 ml-2" />
          </Button>

          {feedback && (
            <div className={`flex items-center gap-2 text-xs p-2 rounded-md ${feedback.type === "success" ? "bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}>
        ...
        {analyzedResult && (
          <div className="mt-4 p-4 bg-background border rounded-lg shadow-sm space-y-3">
            <h4 className="text-sm font-semibold flex items-center gap-2 border-b pb-2">Analysis Results</h4>
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
              <div>
                <span className="text-muted-foreground text-xs block">Category</span>
                <Badge variant="outline" className="mt-1 font-medium">{analyzedResult.category}</Badge>
              </div>
              <div>
                <span className="text-muted-foreground text-xs block">Confidence</span>
                <div className="flex items-center gap-2 mt-1">
                  <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                    <div className={`h-full ${analyzedResult.confidence > 90 ? "bg-green-500" : "bg-amber-500"}`} style={{ width: `${analyzedResult.confidence}%` }} />
                  </div>
              ...
              <div className="col-span-2">
                ...
                {analyzedResult.automatable ? (
                  <Badge className="bg-green-100 text-green-800 hover:bg-green-100 dark:bg-green-900 dark:text-green-200">Yes</Badge>
                ) : (
